...
$query = "select Instructorid, Name, Descrip,Rating_Q1, Rating_Q2, Rating_Q3, Rating_Q4,profile from instructors,instructor_reviews where Name LIKE '%$search_value%' AND instructor_reviews.Instructor_ID = instructors.instructorid";
...
$id = $row['Instructorid'];
$profile = $row['profile'];
$query = "select ROUND(AVG(Rating_Q1),1) as Rating_Q1, ROUND(AVG(Rating_Q2),1) as Rating_Q2, ROUND(AVG(Rating_Q3),1) as Rating_Q3, ROUND(AVG(Rating_Q4),1) as Rating_Q4 from instructor_reviews where instructor_reviews.Instructor_ID = '$id'";
$result = mysqli_query($con, $query) or die(mysqli_error($con));
$row = mysqli_fetch_array($result);
$a1 = $row['Rating_Q1'];
$a2 = $row['Rating_Q2'];
$a3 = $row['Rating_Q3'];
$a4 = $row['Rating_Q4'];
$query = "select comment from instructor_reviews where instructor_reviews.Instructor_ID = '$id'";
$result = mysqli_query($con, $query) or die(mysqli_error($con));
#$comment = $row['comment'];
...
<div class="container">
    <div class="card">
        <h5 class="card-header" id="name"><?php echo $name?></h5>
            <div class="card-body">
                <div class="row">
                    <div class="col-6">
                        <div class="row" class="text-center"><div class="form-group " id="picture" ><img class="prim rounded"  src="assets/img/<?php echo $profile ?>.jpg"></img></div></div>
                    ...
                    <div class="col-6">
                        ...
                        <div class="card mycard">
                            ...
                            $search_value = $_SESSION['Name'];
                            $query = "select instructorid, Name, Descrip,Rating_Q1, Rating_Q2, Rating_Q3, Rating_Q4 from instructors,instructor_reviews where Name LIKE '%$search_value%' AND instructor_reviews.Instructor_ID = instructors.instructorid";
                            $result = mysqli_query($con, $query) or die(mysqli_error($con));
                            $row = mysqli_fetch_array($result);
                            $id = $row['instructorid'];
                            $query = "select comment from instructor_reviews where instructor_reviews.Instructor_ID = '$id'";
